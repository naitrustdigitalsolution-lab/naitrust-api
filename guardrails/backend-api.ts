/**
 * NAITRUST — EXPECTED API CONTRACT (copied from naitrust-web/src/libs/api/backend-api.ts)
 * ================================================================
 * This is the frontend team's reference of every endpoint the frontend
 * expects, with a FULL example request body and a FULL example response
 * body — every field spelled out, nothing abbreviated.
 *
 * IMPORTANT — read this before treating the paths below as the contract to
 * implement: sections 1-11 describe what the CURRENT FRONTEND CODE
 * (`naitrust-web/src/libs/api/endpoints.ts`) calls today, which in several
 * places does NOT match the routes already stubbed in this repo's
 * controllers. Confirmed by direct comparison against
 * `Naitrust.Api/Controllers/*.cs`:
 *
 *   - Only `PublicController` (`/api/Public/*`) is actually implemented.
 *     Every other controller (Auth, Users, Businesses, Transactions,
 *     Milestones, Evidence, Payments, Disputes, Verification, Reputation,
 *     Admin, AI, Notifications, Webhooks) is `NotImplementedException`
 *     stubs — routes exist, bodies don't.
 *   - Where the frontend and these stubs both describe similar concepts,
 *     the shapes diverge: the frontend calls
 *     `/transactions/:id/messages` (chat), `/transactions/:id/tracking/*`
 *     (step-based progress), `/transactions/:id/negotiation/*` (term
 *     proposals), and `/transactions/:id/termination` — none of which are
 *     stubbed here. This repo's `TransactionsController` instead stubs a
 *     simpler lifecycle-action model: `/invite`, `/accept`, `/reject`,
 *     `/terms`, `/approve-terms`, `/fund`, `/deliver`, `/confirm`,
 *     `/cancel`.
 *
 *   See `guardrails/api-contract.md` ("Implementation Status") for the full
 *   note. This is a product/architecture decision to resolve deliberately
 *   with the frontend side (which model to actually build), not something
 *   to silently pick one side of while implementing.
 *
 * Base URL:  {API_BASE_URL}/api
 * Auth:      Bearer <token> in the Authorization header (unless marked "public")
 * Envelope:  every response is this repo's own NaitrustResponse<T> shape —
 *            { statusCode, message, data, isSuccessful } (see
 *            Naitrust.Domain.Models.Dtos.Common.ApiResponse.cs). statusCode
 *            is the HTTP status echoed in the body, message is a
 *            human-readable string, data is the payload (or null),
 *            isSuccessful mirrors 2xx vs 4xx/5xx. Section 12 (Public) is
 *            confirmed live; sections 1-11 are the frontend's expected
 *            shape for when the corresponding controllers are built.
...
//
// NOTE: this section describes what the frontend calls. This repo's
// TransactionsController currently stubs a different, simpler
// lifecycle-action model instead (invite/accept/reject/terms/
// approve-terms/fund/deliver/confirm/cancel, plus separate Milestones,
// Evidence, Disputes, and Payments controllers) — see the file header.
// ================================================================
...
// Five endpoints under /api/Public/* — this section IS live and confirmed,
// implemented in PublicController.cs / PublicFormService.cs. Same
// NaitrustResponse envelope as every other section above.
// ================================================================

/**
 * POST /Public/joinWaitlist (public — no auth)
 *
 * Request body — the frontend modal collects more than the current
 * JoinWaitlistRequest schema accepts; only `name`, `email`, `phone`,
 * `source` are actually bound/persisted today (confirmed against
 * JoinWaitlistRequest.cs and WaitlistEntry.cs) — everything else below is
 * sent by the frontend but currently dropped:
 * {
 *   "name": "Ada Okafor",
 *   "email": "ada.okafor@example.com",
 *   "phone": "+2348012345678",
 ...
 * }
 *
 * Response 201: { "statusCode": 201, "message": "Successfully joined the waitlist.", "data": { "id": "...", "message": "...", "createdAt": "..." }, "isSuccessful": true }
 ...
 *
 * If the richer fields (businessName/userType/transactionRange/
 * transactionNeed/expectations/consent/submittedAt) should be captured,
 * extend JoinWaitlistRequest + WaitlistEntry + PublicFormService.JoinWaitlistAsync
 * accordingly — see guardrails/api-contract.md.
 */
 ...
 * Response 201: { "statusCode": 201, "message": "Message received. We'll get back to you soon.", "data": { "id": "...", "message": "...", "createdAt": "..." }, "isSuccessful": true }
 ...
 * Request body: { "email": "ada.okafor@example.com" }
 * Response 201: { "statusCode": 201, "message": "Successfully subscribed to newsletter.", "data": { "id": "...", "message": "...", "createdAt": "..." }, "isSuccessful": true }
 ...
 * Response 201: { "statusCode": 201, "message": "Thank you for your feedback.", "data": { "id": "...", "message": "...", "createdAt": "..." }, "isSuccessful": true }
 ...
 * Response 201: { "statusCode": 201, "message": "Your concern has been reported. We'll review it promptly.", "data": { "id": "...", "message": "...", "createdAt": "..." }, "isSuccessful": true }
